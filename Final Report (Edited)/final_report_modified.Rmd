...
**Weekday vs. Weekends**
  ...
  - While overall entry and exit volumes go down over the weekends (defined as Saturday and Sunday)  by “INSERT QUANTIFICATION”, night-time intervals such as 12AM-4AM and 8PM-12AM experience an uptick with the arrival of the weekend by “INSERT QUANTIFICATION”
...
```{r fig.width=12, fig.height=24, echo=FALSE}
...
```{r, fig.height=5, fig.width=8, message=FALSE, echo=FALSE}
...
```{r, fig.height=5, fig.width=14, message=FALSE, echo=FALSE}
...
One issue we ran into the We wanted to explore turnstile data with time of a day. However, the timestamp in the original turnstile data was inconsistent, which made it difficult to analyze the trend of subway usage over interval. As lengths between times were often around 3 to 4 hours, we decided that the most granular level of aggregation that least sacrified accuracy, was entry/exit volumes on 4-hour intervals, starting at midnight. This way, there were only 6 intervals that could cleanly display intra-day passenger traffic patterns.
...
NOTE: the interactive map may fail to load at once. Please try refreshing the page at least once or twice before jumping to deduct points.
  - 
  - 
  - 
