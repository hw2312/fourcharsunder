...
# Take this David
```{r}
data0 <- turnstile %>% select(day, is_holiday, entry_volume, exit_volume) %>% group_by(day, is_holiday)

# Change the value of "day" to "Weekday" or "Weekend"
day_list = c("Monday","Tuesday","Wednesday","Thursday","Friday","Saturday","Sunday")
for (i in day_list){
  if (i != "Saturday" & i != "Sunday"){
    data0[,"day"] <- data.frame(lapply(data0[,"day"], function(x) {gsub(i, "Weekday", x)}))
  }
  else {
    data0[,"day"] <- data.frame(lapply(data0[,"day"], function(x) {gsub(i, "Weekend", x)}))
  }
}

data0 <- data0 %>% ungroup() %>% mutate(day = if_else(day == "Weekday" & is_holiday == "False", "Weekday", "Weekend"))

data0 <- data0 %>% group_by(day) %>% summarise(avg_entry = mean(entry_volume), avg_exit = mean(exit_volume))
```

```{r}
ggplot(data0, aes(y = avg_entry, x = day)) + 
  geom_col(col='#0072B2', fill="#66CC99")  + ylab("Entry Count") + xlab("Day")  + coord_flip()
```

```{r}
ggplot(data0, aes(y = avg_exit, x = day)) + 
  geom_col(col='#0072B2', fill='#E69F00')  + ylab("Exit Count") + xlab("Day")  + coord_flip()
```
...
### 4.3 Subway Riders vs. Taxi Riders

```{r, echo=FALSE}
entry = read.csv("data/entry.csv")
...
```
This plot shows the number of subway entries against taxi pick-ups by time of the day. Assuming the total number of people use subway or taxi is constant, the slopes represent which mode of transportation people prefer. On average, people use taxi more often during morning rush hour (4 am - 8 am) than evening rush hour (4 pm - 8 pm). We can see that people prefer taxi when they have to get to places on time. 

```{r, echo=FALSE}
exit = read.csv("data/exit.csv")
...
Similarly, this plot show the number of subway exits against taxi drop-offs by time of the day. Overall, the differences between entries and exits are not apparent except for morning times. This plot has steeper lines in the morning. By combining observations together, we can conclude that Manhattan residents tend to use taxi more often than commuters from outside of Mahattan in the morning and vice versa in the evening. It is also interesting to note that the slopes for entries and exists are almost identical during 12PM-4PM and 8PM-12AM. This tells us that during these time periods, people equally use taxi and subway to get to places within Mahattan area.
...
```{r}
unique_time = read.csv("data/unique_time.csv")
...
```
* To analyze how the values of timestamp are distributed more efficiently, we preprocessed the original data such that each row in TimeStamp column contain unique value of timestamp and each row in Count column has corresponding count of timestamp.
* We generated the graph above is to show that we have numerous unique value of timestamp in our original data.
* The plotted histogram above only with 100 sampled timestamp out of 85394 unique timestamps.
* We wanted to explore turnstile data with time of a day. However, the timestamp in the original turnstile data had too many values, which made it difficult to analyze the trend of subway usage over interval.
